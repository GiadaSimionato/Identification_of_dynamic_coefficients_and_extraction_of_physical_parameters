...
%q_j = simplify((1/wf)*(A(j,:)*aux_sin' - B(j,:)*aux_cos') + L*q0(j)); % trajectory for joint positions
...
for j=1:n_joints
    ...
    ddqf(j, 1:int32(margin/s_rate)-1) = awgn(ddq(j, 1:int32(margin/s_rate)-1), snr, 'measured');  
